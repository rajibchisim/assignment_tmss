...
<div class="container flex items-center h-screen mx-auto">
    <div class="flex flex-grow max-w-sm mx-auto overflow-hidden bg-white rounded-lg shadow-lg lg:max-w-4xl">
        ...
        <div class="w-full px-6 py-8 md:px-8 lg:w-1/2">
            <h2 class="text-2xl font-semibold text-center text-gray-700">TMSS Student portal</h2>
            <p class="text-xl text-center text-gray-600"></p>
            <div class="flex items-center justify-between mt-4">
                <span class="w-1/5 border-b lg:w-1/4"></span>
                <p class="text-xs text-center text-gray-500 uppercase hover:underline">Register new account</p>
                <span class="w-1/5 border-b lg:w-1/4"></span>
            </div>
            <form @submit.prevent="validateForm" class="mt-6">
                <div class="relative mt-4">
                    <label class="block mb-1 text-sm font-medium text-gray-600" for="name">Full Name</label>
                    <input id="name" name="name" v-validate="'required'" v-model="formData.name" class="block w-full px-4 py-1 text-gray-700 bg-white border rounded-md focus:border-blue-400 focus:ring-opacity-40 focus:outline-none focus:ring focus:ring-blue-300" type="text">
                    <span class="absolute right-0 text-xs text-red-500 -bottom-4">{{ errors.first('name') }}</span>
                </div>
                <div class="relative mt-4">
                    <label class="block mb-1 text-sm font-medium text-gray-600" for="email">Email Address</label>
                    <input id="email" name="email" v-validate="'required|email'" v-model="formData.email" class="block w-full px-4 py-1 text-gray-700 bg-white border rounded-md focus:border-blue-400 focus:ring-opacity-40 focus:outline-none focus:ring focus:ring-blue-300" type="email">
                ...
                <div class="relative mt-4">
                    ...
                    <input id="password" name="password" v-validate="'required|min:6'" ref="password" v-model="formData.password" class="block w-full px-4 py-1 text-gray-700 bg-white border rounded-md focus:border-blue-400 focus:ring-opacity-40 focus:outline-none focus:ring focus:ring-blue-300" type="password">
                    <span class="absolute right-0 text-xs text-red-500 -bottom-4">{{ errors.first('password') }}</span>
                </div>
                <div class="relative mt-4">
                    <label class="block mb-1 text-sm font-medium text-gray-600" for="password_confirmation">Confirm Password</label>
                    <input id="password_confirmation" v-validate="'required|confirmed:password'" v-model="formData.password_confirmation" name="password_confirmation" class="block w-full px-4 py-1 text-gray-700 bg-white border rounded-md focus:border-blue-400 focus:ring-opacity-40 focus:outline-none focus:ring focus:ring-blue-300" type="password">
            ...
            <div class="flex items-center justify-between mt-6">
                ...
                <router-link :to="{ name: 'login' }" class="text-xs text-gray-500 uppercase hover:underline">Already registered?</router-link>
                <span class="w-1/5 border-b md:w-1/4"></span>
            </div>
        </div>
    </div>
